...
const App = () => {
  ...
  // Only use location hook if auth is initialized to prevent timing issues
  let location;
  try {
    location = useLocation();
  } catch (error) {
    // Fallback if router context not ready
    location = { pathname: "/" };
  }
  ...
  const hideBottomNav = pathsWithoutNav.some((path) =>
    ...
    <div className="bg-background text-foreground min-h-screen font-sans">
      <main className={isLoggedIn && !hideBottomNav ? "pb-24" : ""}>
        <Routes>
          ...
          <Route
            path="*"
            element={<Navigate to={isLoggedIn ? "/" : "/profile"} replace />}
          />
        </Routes>
      </main>
